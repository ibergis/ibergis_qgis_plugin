...
BEGIN
    UPDATE inp_conduit
        SET node_2 = (
            SELECT v_node.code
            FROM v_node
            WHERE ST_Intersects(ST_Buffer(v_node.geom, 0.1), ST_EndPoint(NEW.geom))
            LIMIT 1
        ),
        node_1 = (
            SELECT v_node.code
            FROM v_node
            WHERE ST_Intersects(ST_Buffer(v_node.geom, 0.1), ST_StartPoint(NEW.geom))
            LIMIT 1
        )
    WHERE fid = NEW.fid;
END;
...
BEGIN
    UPDATE inp_weir
        SET node_2 = (
            SELECT v_node.code
            FROM v_node
            WHERE ST_Intersects(ST_Buffer(v_node.geom, 0.01), ST_EndPoint(NEW.geom))
            LIMIT 1
        ),
        node_1 = (
            SELECT v_node.code
            FROM v_node
            WHERE ST_Intersects(ST_Buffer(v_node.geom, 0.01), ST_StartPoint(NEW.geom))
            LIMIT 1
        )
    WHERE fid = NEW.fid;
END;
