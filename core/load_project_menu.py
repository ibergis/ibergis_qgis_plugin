...
from .ui.ui_manager import DrLoadMenuUi
from .. import global_vars


class DrMenuLoad(QObject):
    ...
    def read_menu(self, project_loaded):
        ...
        if project_loaded:
            ...
            action_help_shortcut = tools_dr.get_config_parser("actions_shortcuts", "shortcut_help", "user", "init", prefix=False)
            if not action_help_shortcut:
                tools_dr.set_config_parser("actions_shortcuts", "shortcut_help", f"{action_help_shortcut}", "user", "init",
                                           prefix=False)
            action_help.setShortcuts(QKeySequence(f"{action_help_shortcut}"))
            action_help.triggered.connect(tools_dr.open_dlg_help)
        ...
        if project_loaded:
            ...
            action_reset_plugin_shortcut = tools_dr.get_config_parser("actions_shortcuts", "shortcut_reset_plugin",
                "user", "init", prefix=False)
            if not action_reset_plugin_shortcut:
                tools_dr.set_config_parser("actions_shortcuts", "shortcut_reset_plugin",
        ...
        log_sql_shortcut = tools_dr.get_config_parser("actions_shortcuts", "shortcut_toggle_log_db", "user", "init", prefix=False)
        if not log_sql_shortcut:
            tools_dr.set_config_parser("actions_shortcuts", "shortcut_toggle_log_db", f"{log_sql_shortcut}", "user",
                "init", prefix=False)
        ...
        if global_vars.plugin_dir:
            icon_path = f"{icon_folder}{os.sep}dialogs{os.sep}24x24{os.sep}108.png"
        ...
        icon_path = f"{icon_folder}{os.sep}dialogs{os.sep}24x24{os.sep}105.png"
        folder_icon = QIcon(icon_path)
        ...
        if global_vars.user_folder_dir:
            ...
            icon_path = f"{icon_folder}{os.sep}dialogs{os.sep}24x24{os.sep}102.png"
            folder_icon = QIcon(icon_path)
    ...
    def _open_manage_file(self):
        ...
        self.dlg_manage_menu = DrLoadMenuUi()
        tools_dr.load_settings(self.dlg_manage_menu)
        ...
        self.dlg_manage_menu.rejected.connect(partial(tools_dr.close_dialog, self.dlg_manage_menu))

        tools_qgis.restore_cursor()

        msg = ("Changes on this page are dangerous and can break Drain plugin in various ways. \n"
        ...
        # Open dialog
        tools_dr.open_dialog(self.dlg_manage_menu, dlg_name='load_menu', title="Advanced Menu")

    def _reset_position_dialog(self):
        """ Reset position dialog x/y """
        tools_dr.reset_position_dialog(True)
    ...
    def _fill_tbl_config_files(self):
        ...
        for file in files:
            ...
            project_types = tools_dr.get_config_parser('system', 'project_types', "project", "drain")
    ...
    def _set_config_value(self, item, column):
        ...
        if column == 2:
            ...
            if value == '':
                value = "None"
            tools_dr.set_config_parser(section, parameter, value, file_name=file_name, prefix=False, chk_user_params=False)
    ...
    def _set_log_sql(self):
        ...
        log_sql = tools_dr.get_config_parser("log", "log_sql", "user", "init", False, get_none=True)
        if log_sql in ("False", "None"):
            msg = "Variable log_sql from user config file has been enabled."
            tools_dr.set_config_parser("log", "log_sql", "True", file_name="init", prefix=False)
        else:
            msg = "Variable log_sql from user config file has been disabled."
            tools_dr.set_config_parser("log", "log_sql", "None", file_name="init", prefix=False)
    ...
    def _reset_plugin(self):
        ...
        self._reload_layers()  # Execute DrProjectLayersConfig thread
