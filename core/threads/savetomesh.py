...
def apply_boundary_conditions_to_mesh(mesh: mesh_parser.Mesh, bc_scenario: str, dao: DrGpkgDao):
    ...
    for edge, (pol_id, side) in boundary_edges.items():
        coords = [mesh.vertices.loc[vert, ["x", "y"]].to_numpy() for vert in edge]
    ...
    sql = f"""
        ...
        WHERE bscenario = '{bc_scenario}'
    """
    rows = dao.get_rows(sql)
    if rows is None:
        raise ValueError("No boundary conditions found for this scenario")
    ...
    def get_timeseries(timeseries_id):
        ...
        rows = dao.get_rows(sql)
        if rows is None:
            raise ValueError(f"Timeseries idval {timeseries_id} is empty.")
    ...
    for feature in result_layer.getFeatures():
        ...
        mesh.boundary_conditions[(feature["pol_id"], feature["side"])] = bc_dict[feature["bc_fid"]]
...
class DrSaveToMeshTask(DrTask):
    ...
    def run(self):
        ...
        try:
            ...
            apply_boundary_conditions_to_mesh(self.mesh, self.bc_scenario, dao)
