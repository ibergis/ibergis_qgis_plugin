...
def dump_new(mesh, mesh_fp, roof_fp, losses_fp):
    mesh_fp.write("MATRIU\n")
    mesh_fp.write(f"  {len(mesh['polygons'])}\n")
...
def dumps_new(mesh):
    with (
        ...
        dump_new(mesh, mesh_buffer, roof_buffer, losses_buffer)
        return mesh_buffer.getvalue(), roof_buffer.getvalue(), losses_buffer.getvalue()
...
def load_new(mesh_fp: io.StringIO, roof_fp=None, losses_fp=None):
...
def loads_new(mesh_string, roof_string="", losses_string=""):
    with (
        io.StringIO(mesh_string) as mesh_file,
        io.StringIO(roof_string) as roof_file,
        io.StringIO(losses_string) as losses_file,
    ):
        return load_new(mesh_file, roof_file, losses_file)
