...
def delete_object(**kwargs):
    ...
    answer = tools_qt.show_question(msg, title, list_object_id)
    if answer:
...
def set_column_visibility(**kwargs):
    ...
    try:
        ...
        msg = "Error setting column visibility: {0} --> {1}"
...
def set_column_multiline(**kwargs):
    ...
    try:
        ...
        msg = "Error setting column multiline: {0} --> {1}"
...
def set_read_only(**kwargs):
    ...
    try:
        ...
        msg = "Error setting field {0}: {0} --> {1}"
        msg_params = ("readOnly", type(e).__name__, e,)
        tools_log.log_info(msg, msg_params=msg_params)
        return
    # Get layer config
