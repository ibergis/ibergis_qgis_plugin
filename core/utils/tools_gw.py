...
def remove_deprecated_config_vars():
    ...
    project_types = get_config_parser('system', 'project_types', "project", "drain").split(',')
    ...
    vars = get_config_parser('system', 'deprecated_section_init', "project", "drain")
    if vars is not None:
    ...
    vars = get_config_parser('system', 'deprecated_section_session', "project", "drain")
    if vars is not None:
        for var in vars.split(','):
            section = var.split('.')[0].strip()
    ...
    init_parser.read(path)
    vars = get_config_parser('system', 'deprecated_vars_init', "project", "drain")
    if vars is not None:
        for var in vars.split(','):
            section = var.split('.')[0].strip()
            if not init_parser.has_section(section):
    ...
    session_parser.read(path)
    vars = get_config_parser('system', 'deprecated_vars_session', "project", "drain")
    if vars is not None:
        for var in vars.split(','):
            section = var.split('.')[0].strip()
            if not session_parser.has_section(section):
