...
from . import tools_dr


class DrSnapManager(object):
    ...
    def add_point(self, vertex_marker):
        ...
        # Snapper
        tools_dr.disconnect_signal('snap_managers', f'{hash(self)}_ep_canvasClicked_get_xy')
        emit_point = QgsMapToolEmitPoint(global_vars.canvas)
        global_vars.canvas.setMapTool(emit_point)
        tools_dr.connect_signal(global_vars.canvas.xyCoordinates, partial(self._get_mouse_move, vertex_marker),
                                'snap_managers', f'{hash(self)}_xyCoordinates_get_mouse_move')
        tools_dr.connect_signal(emit_point.canvasClicked, partial(self._get_xy, vertex_marker, emit_point),
    ...
    def _get_xy(self, vertex_marker, emit_point, point):
        ...
        tools_dr.disconnect_signal('snap_managers', f'{hash(self)}_ep_canvasClicked_get_xy')
        tools_dr.disconnect_signal('snap_managers', f'{hash(self)}_xyCoordinates_get_mouse_move')
