    ...
    if all([type(dt_val) in [QDate, QTime] for dt_val in dt_list]):
        ...
        if not any([x.isNull() for x in dt_val_list]):
            ...
            feedback.pushWarning(
                f'Timeseries \"{ts_name}\" {dt_type} column was derived from strings (assumed format: {d_f})'
            )
        else:
            raise QgsProcessingException(f'{str(ts_name)}: column {dt_type} could not be converted properly. Tested formats: {dt_formats}')
    return dt_val_list
