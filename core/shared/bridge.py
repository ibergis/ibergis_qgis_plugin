...
class PlotColors:
    """ Class to store plot colors """
    # Error
    ERROR_TEXT = 'black'
    ERROR_BACKGROUND = '#a11d00'
    # Info
    INFO_TEXT = 'black'
    # Bridge
    TOPELEV_MARKER = "x"
    LOWELEV_MARKER = "x"
    TOPELEV_COLOR = '#001246'
    LOWELEV_COLOR = '#003dec'
...
class DrBridge:
    ...
    def manage_bridge(self, bridge: Optional[QgsFeature] = None, is_new: bool = False):
        ...
        # Fill table
        self.fill_table(self.dialog.tbl_bridge_value, bridge, is_new)
    ...
    def _manage_table(self, dialog, is_add: bool):
        ...
        if is_add:
            ...
            if dialog.tbl_bridge_value.rowCount() == 0:
                ...
                dialog.tbl_bridge_value.setItem(0, 1, QTableWidgetItem('0.0'))
                dialog.tbl_bridge_value.setItem(0, 2, QTableWidgetItem('0.0'))
    ...
    def fill_table(self, widget, bridge, is_new: bool = False):
        """ Fill bridge table """
        ...
        if is_new:
            ...
            widget.setItem(0, 1, QTableWidgetItem('0.0'))
            widget.setItem(0, 2, QTableWidgetItem('0.0'))
            widget.setItem(0, 3, QTableWidgetItem('100'))
            widget.insertRow(1)
            widget.setItem(1, 0, QTableWidgetItem(f"{round(bridge.geometry().length(), 3)}"))
            widget.setItem(1, 1, QTableWidgetItem('0.0'))
            widget.setItem(1, 2, QTableWidgetItem('0.0'))
    ...
    def _update_bridge_profile_plot(self, table):
        ...
        if not incoherent_top_low_elevs:
            ...
            self.plot_widget.axes.text(0.5, 0.5, text,
                            ...
                            fontsize=12, color=PlotColors.ERROR_TEXT, zorder=5, fontweight='bold', bbox=dict(facecolor=PlotColors.ERROR_BACKGROUND, alpha=0.9))

        # Only plot if we have valid data
        if len(distances) < 2:
            self.plot_widget.axes.text(0.5, 0.5, 'Add at least 2 points\nto see the profile',
                                     ha='center', va='center', color=PlotColors.INFO_TEXT, transform=self.plot_widget.axes.transAxes)
        ...
        if dem_distances and dem_values:
            label = 'DEM Ground'
            self.plot_widget.axes.plot(dem_distances, dem_values, color=PlotColors.DEM_COLOR, linewidth=2, label=label, alpha=0.7, zorder=1)

        # Plot the bridge lines on top
        self.plot_widget.axes.plot(processed_distances, processed_topelevs, color=PlotColors.TOPELEV_COLOR, linewidth=1.5, label='Top Elevation', marker=PlotColors.TOPELEV_MARKER, markersize=3, zorder=3)
        self.plot_widget.axes.plot(processed_distances, processed_lowelevs, color=PlotColors.LOWELEV_COLOR, linewidth=1.5, label='Low Elevation', marker=PlotColors.LOWELEV_MARKER, markersize=3, zorder=3)

        # Fill area between top and low elevation and between ground and 0
        self.plot_widget.axes.fill_between(processed_distances, processed_lowelevs, processed_topelevs, alpha=0.5, color=PlotColors.ELEVATION_FILL_COLOR, zorder=2)
        self.plot_widget.axes.fill_between(dem_distances, dem_values, 0, alpha=0.6, color=PlotColors.GROUND_COLOR, zorder=2)

        # Customize plot
        self.plot_widget.axes.set_xlabel('Distance (m)')
        self.plot_widget.axes.set_ylabel('Elevation (m)')
        self.plot_widget.axes.set_title('Bridge Profile')
        self.plot_widget.axes.legend(loc=3, labelspacing=0.1, fontsize=8)
        self.plot_widget.axes.grid(True, alpha=0.3)
        ...
        if processed_distances:
            ...
            self.plot_widget.axes.set_ylim(((y_center - y_range/2 - y_margin) - 10), y_center + y_range/2 + y_margin)
    ...
    def _show_empty_plot(self, text: str = '', is_error: bool = False):
        ...
        if is_error:
            ...
            self.plot_widget.axes.text(0.5, 0.5, text,
                            ha='center', va='center', transform=self.plot_widget.axes.transAxes,
                            fontsize=12, color=PlotColors.INFO_TEXT)
